...
describe("Marketing Consent API boundary", () => {
  ...
  const service: MarketingConsentService = Object.freeze({ grant });
  ...
  it("rejects unknown Marketing paths without invoking service", async () => {
    grant.mockClear();
    const response = await handler(
      request(validBody(), {
        path: "/api/marketing-consent/v1/withdrawals",
      }),
    );
    expect(response.status).toBe(400);
    expect(response.redirected).toBe(false);
