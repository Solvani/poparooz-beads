...
Status: **Generator production baseline retained; Email Gate P3-A03-E04-A08 completed, production verified, frozen, and closed through A08-A01**

Baseline version: **3.14**
...
- [`POPAROOZ_EMAIL_GATE_OTP_V2_CONTRACT.md`](POPAROOZ_EMAIL_GATE_OTP_V2_CONTRACT.md): authorized separate OTP V2 authority; retains frozen strict eight-digit V1 for legacy clients/challenges and defines strict six-digit V2, versioned routes, trusted challenge discrimination, deterministic derivation, and Worker-first rollout. The commit containing the document establishes repository implementation authority; deployment, verification, and master acceptance remain separate gates.
...
The Marketing Consent MC-A00 authority freezes the product, data, and privacy
requirements. MC-A01 synchronized that authority and is completed and closed.
...
MC-A02-A02-H01 correction became effective through its successful exact
three-file governance commit. Its before/after transition states are retained
in the frozen contract as historical provenance and do not describe the current
runtime. Current Marketing authority is the accepted MC-A02-E02-P03
withdrawal-only production state summarized below.
...
Generator deployment/acceptance bullets retain the earlier MVP evidence. The
separate Email Gate production acceptance is recorded below at the latest
...
- A01: **P3-A03-E04-A08-A01 — Bounded Production Acceptance Verification**
...
- A00 audited range: **Exactly six implementation commits after the reconciliation baseline**
...
- Turnstile: **Retry recovery preserves the product contract; the earlier Siteverify redirect-policy correction resolved the historical transport failure**
- Live Shopify sandbox: **allow-scripts allow-same-origin allow-downloads allow-forms; form submission restored with popup/top-navigation permissions absent**
...
- OTP authority evolution: **EMAIL-GATE-OTP-UX-P01 ACTIVE / V1 FROZEN AT 8 DIGITS FOR LEGACY COMPATIBILITY / V2 AUTHORIZED AND IMPLEMENTED AT 6 DIGITS FOR NEW ISSUANCE AND CLIENT UX / NOT YET DEPLOYED OR ACCEPTED**
- V1/V2 discrimination: **Separate versioned request and verification routes plus trusted `delivery_payload_version`; submitted code length never selects protocol behavior; D1 migration 0**
...
- Historical production stage: **MC-A02-E02-P04 — Controlled Marketing Lifecycle E2E**
- Accepted production stage: **MC-A02-E02-P05 — Marketing Grant Production Completion**
...
- P04 status: **CLOSED / HISTORICAL WITHDRAWAL-ONLY FINAL DISPOSITION**
- P05 status: **PASS / MARKETING GRANT + WITHDRAWAL PRODUCTION LIFECYCLE VERIFIED / READY FOR FINAL GOVERNANCE CLOSURE**
- Full Marketing lifecycle: **G1 -> W1 -> G2 -> W2 / PROVEN / PASS**
- Marketing functional implementation authority: `a547d01ffad9294de934f00c9a20cbc7105ecd99`
- Accepted P05 lifecycle deployment: `b5425c91-110f-448e-9758-f06cc7f28d8e`
- Marketing production configuration: `VITE_MARKETING_CONSENT_ENABLED=true`; `VITE_MARKETING_WITHDRAWAL_ENABLED=true`
- Current customer behavior: **Marketing checkbox enabled and optional; grant enabled; withdrawal self-service enabled; `/unsubscribe` active**
- P05 final QA state: **subscription withdrawn / state_version 4 / granted -> withdrawn -> granted -> withdrawn / rollback 0**
- Marketing integration qualification: **No Resend Contacts/Audiences, campaigns, Shopify Marketing mutation, CRM sync, or Marketing sending integration is implemented or authorized**
...
- PatternCosting Generator status: **V2.1 CLOSED**
- PatternCosting implementation authority: `a0005ecf885db2d458679b20ec7c6006d7b12b79`
- PatternCosting next action: **No fresh D00 rerun is current; reopening requires separate authority**
- Evidence qualification: **P04 remains the historical withdrawal-only disposition. P05 is the later accepted lifecycle authority and proves G1 -> W1 -> G2 -> W2 without rewriting P03/P04 history. This docs reconciliation does not rerun production or change runtime behavior.**
- Repository authority entering this docs-only reconciliation: `a547d01ffad9294de934f00c9a20cbc7105ecd99`; **the docs-only commit containing this text becomes repository and production source authority after its normal `main` push without replacing the Marketing functional implementation authority**
...
The frozen and published MC-A02 runtime, API, and persistence contract is in
...
P04 remains CLOSED as the historical withdrawal-only final disposition. Current
Marketing production authority is the later accepted MC-A02-E02-P05 lifecycle:
Marketing Grant and Withdrawal are ENABLED, G1 -> W1 -> G2 -> W2 is PROVEN /
PASS, the final QA subscription is withdrawn at state_version 4, and rollback
count is zero. Marketing persistence does not imply a provider or sending
integration. PatternCostingExportV2 v2.1.0 remains a separate frozen Costing
contract dependency, and its Generator implementation is CLOSED at authority
`a0005ecf885db2d458679b20ec7c6006d7b12b79`. No fresh D00 rerun is the current
next action; reopening or adjacent business-data mutation requires separate
authority.
...
supersedes that documentation regression. Published history remains intact.

The accepted flow preserves the frozen privacy boundary: images, Pattern data,
and PNG generation remain browser-local; Email Gate creates no account or
Shopify customer; marketing consent remains optional and separate; OTP
verification remains server-authoritative; and secrets remain Cloudflare
bindings outside the repository. Scheduled cleanup over time, future key
rotation, provider outages, and browser/platform changes remain non-blocking
operational concerns rather than reasons to keep A08 open.
