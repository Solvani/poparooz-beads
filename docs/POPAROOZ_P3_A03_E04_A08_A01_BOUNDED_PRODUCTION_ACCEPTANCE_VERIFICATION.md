...
governance regression that downgraded the already accepted evidence in this
record. It did not alter Worker or production runtime behavior. The R03 forward
correction supersedes that documentation regression. Neither `f8852d2` nor the
earlier historical qualifications are erased or rewritten.
...
The original Windows `curl` reproduction was a quoting/serialization false
negative. A subsequent Node `JSON.stringify` reproduction returned the
expected HTTP 400 with `schemaVersion: 1` and `result: version_unsupported`.
The original `curl` invocation therefore does not support an open parser-defect
claim. This record does not infer evidence beyond that established
reproduction.
...
**APPROVED / FROZEN**. Its implementation release was granted separately after
Email Gate A08 closure. Marketing Consent runtime implementation and MC-A01
authority synchronization remain **NOT STARTED**; no Marketing Consent code,
D1 schema change, or provider connection is part of R03.

Ops Dashboard v1 remains **HOLD** and is not released by this correction.
